...
export async function getUserBooks() {
  ...
  try {
    ...
    // Get the user's books with cover images
    const { data: books, error } = await supabase
      .from("coloring_books")
      .select(`
        *,
        cover:cover_id(id, image_url)
      `)
      .eq("user_id", user.id)
    ...
    console.log(`Fetched ${books?.length || 0} books for user`)
...
export async function getBookById(bookId: string) {
  ...
  try {
    ...
    const { data: book, error } = await supabase
      ...
      .select(`
    *,
    cover_images!coloring_books_cover_id_fkey(id, image_url)
  `)
      .eq("id", bookId)
      .single()
